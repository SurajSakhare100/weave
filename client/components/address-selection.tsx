    ...
    <div className="min-h-screen bg-[#faf9f7] p-6">
      <div className="max-w-2xl mx-auto">
          ...
          {addresses.map((address) => (
            <div key={address.id} className="relative">
              <div className="flex items-start space-x-4">
                {/* Custom Radio Button */}
                <button onClick={() => setSelectedAddress(address.id)} className="mt-1 flex-shrink-0">
                  <div className="w-5 h-5 rounded-full border-2 border-[#8b7355] flex items-center justify-center">
                    {selectedAddress === address.id && <div className="w-3 h-3 rounded-full bg-[#8b7355]"></div>}
                  </div>
                </button>
              ...
              {selectedAddress === address.id && (
                <div className="mt-6 ml-9">
                  <Button
                    className="bg-[#e91e63] hover:bg-[#c2185b] text-white px-6 py-2.5 rounded-md font-medium"
                    onClick={() => console.log(`Delivering to ${address.name}`)}
                  >
        ...
        <div className="mt-16 text-center">
          <button
            className="text-[#e91e63] hover:text-[#c2185b] font-medium text-lg underline underline-offset-2"
