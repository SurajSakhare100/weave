...
const CartItem = ({ item, onQuantityChange, onRemove, isUpdating = false }) => {
  ...
  return (
    <div className={` flex flex-row items-center  rounded-xl outline-none sm:rounded-2xl border-1 border-[#FFF6EF] gap-4 sm:gap-6 lg:gap-8 overflow-hidden bg-white shadow-sm hover:shadow-md transition-shadow duration-300 ${isUpdating ? 'opacity-50' : ''}`}>
      {/* Product Image */}
      <div className=" bg-[#FFF6EF] p-2 sm:p-4 w-auto">
        <div className="relative aspect-square   h-24 sm:w-24 sm:h-24 lg:w-28 lg:h-28">
          <Image
            src={productImage}
            alt={productName}
            fill
      ...
      {/* Product Details */}
      <div className="flex flex-row items-start justify-between gap-3 sm:gap-4 py-2 sm:py-4 pr-4 w-full">
        <div className="flex-1 min-w-0">
          <h3 className="font-bold text-sm sm:text-lg lg:text-xl text-wrap text-[#6c4323] line-clamp-2 mb-1 sm:mb-2">
            {productName.length > 15 ? productName.slice(0, 15) + '...' : productName}
          </h3>
          <p className="text-sm sm:text-xl font-semibold text-[#6c4323] mb-1">
        ...
        <div className="flex   items-center self-end justify-between sm:justify-end gap-1.5 sm:gap-3">
          {/* Quantity Controls */}
          <div className="flex items-center  sm:w-auto  rounded outline outline-[0.60px] outline-offset-[-0.60px] outline-tags inline-flex justify-center items-center gap-1.5 border border-[#EF3B6D] rounded-sm px-3 py-1 sm:px-3  sm:py-2 gap-1.5 sm:gap-3 bg-white">
